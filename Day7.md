...
可选择紧接在另一元素后的一个元素（也**只选择这一个**），且**二者必须是有相同父元素的**。

```html
h1 + p {margin-top:50px;}//如这里只选p不选h1
```
...
不希望选择任意的后代元素，而是希望缩小范围，只选择**子元素**（！！！**注意只能对子元素，孙子元素不行，故这也是和后代选择器的区分点**）所以是必须严格**按路径**的**子元素**。
...
- 只有有某个属性（或者多个）：
  ...
  ```css
  *[title] {color:red;}//包含标题（title）的所有元素
  ```

  ```css
  a[href] {color:red;}//有 href 属性的锚（a元素）
  ```

  ```css
  a[href][title] {color:red;}//同时有href和title属性的a
  ```

- 根据属性值选择，可以多个条件选择，此时是**严格匹配**，必须**一字不差**。
...
- 模糊匹配：
  ...
  | 选择器                                                       | 描述                                                         |
  | :----------------------------------------------------------- | :----------------------------------------------------------- |
  | [[*attribute*\]](https://www.w3school.com.cn/cssref/selector_attribute.asp) | 有这个属性即可。                                             |
  | [[*attribute*=*value*\]](https://www.w3school.com.cn/cssref/selector_attribute_value.asp) | 值严格匹配，一字不差。                                       |
  | [[*attribute*~=*value*\]](https://www.w3school.com.cn/cssref/selector_attribute_value_contain.asp) | 包含value这个独立出来的单词，如"true value"是成立的，"true_value"就不成立了。 |
  | [[attribute*=value\]](https://www.w3school.com.cn/cssref/selector_attr_contain.asp) | 包含value这一段就可，范围比"~="更大。                        |
  | [[*attribute*^=*value*\]](https://www.w3school.com.cn/cssref/selector_attr_begin.asp) | 以value开头                                                  |
  | [[*attribute*$=*value*\]](https://www.w3school.com.cn/cssref/selector_attr_end.asp) | 以value结尾                                                  |
  | [[*attribute*\|=*value*\]](https://www.w3school.com.cn/cssref/selector_attribute_value_start.asp) | 以"value-"开头的，"-"必不可少                                |
...
they act as if you had added a whole new HTML element into the markup, rather than applying a class to existing elements.是增加了一个元素
